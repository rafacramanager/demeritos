...
<html lang="es">
  ...
  <form method="GET" class="row g-2 mb-4 p-3 bg-white rounded shadow-sm">
    ...
    <table id="tablaEstudiantes" class="table table-striped table-hover align-middle" style="width:100%">
      ...
      <tbody>
        <?php
// Supongamos que en la sesión tienes guardado el tipo de usuario:
$usuario = $_SESSION['usuario']; // nombre de usuario actual
//$tipo_usuario = $_SESSION['tipo_usuario']; // puede ser 'superuser' o 'regular'

// Construir la consulta base
if ($usuario === 'Rafael') {
    // El superuser puede ver todos los registros
    $sql = "SELECT * FROM estudiantes WHERE estado = 'Activo'";
} else {
    // Los usuarios regulares solo ven su propio centro educativo
